...
def run_natural_group(init_params, s_0s, exp_dir='exp_dir', exp_id=None):
    df = 0
    for s_0 in s_0s:
        init_params['s_0'] = s_0
        print("running natural experiment, s_0: ", init_params['s_0'])
        df = run_natural(init_params)

    return df


def run_natural(init_params):
    start_exp = time.time()
    ...
    # run n trials for each experiment (just 1 for natural)
    # tmp = pd.read_csv('gene_labels.csv')
    # classes_true = tmp.iloc[:, -1:].values.flatten()
    # print(classes_true.shape)

    # tmp2 = pd.read_csv('gene_data.csv')
    # print(tmp2.shape)
    # X = tmp.iloc[:, 1:].values
    classes_true = init_params['classes']
    X = init_params['vals']
    ...
    centers_init = initcenters(X, k, random_state=0) #want to keep consistent center initialization
...
if __name__ == "__main__":
    ...
    tmp = pd.read_csv('3D_spatial_network.csv')
    tmp = tmp.iloc[:lowDsamps]
    road_vals = tmp.iloc[:, -3:].values
    road_classes = tmp.iloc[:, :1].values.flatten()
    unique_classes = np.unique(road_classes)
    mapping = {cls: i for i, cls in enumerate(unique_classes)}
    road_classes = np.vectorize(mapping.get)(road_classes)

    print(unique_classes.size)
   
    tmp = pd.read_csv('gene_labels.csv')
    gene_classes = tmp.iloc[:, -1:].values.flatten()
   # print(classes_true.shape)

    tmp2 = pd.read_csv('gene_data.csv')
   # print(tmp2.shape)
    gene_vals = tmp.iloc[:, 1:].values
    ...
    init_params_highDnatural = {
            'n_trials': 1,
            'bregman_dist': 'multiomial',
            'vals' : gene_vals,
            'classes' : gene_classes,
            'data_params': {
                'n_samples': highDsamps,
                'n_features': highDd, 
                'center_box': (1, 40),
                'center_coordinates': np.array([[10]*d, [20]*d, [40]*d]),
                'centers': k,
    ...
    for s_0 in s_0_list:
        ...
        k = lowDk
        result = run_natural_group(init_params_lowDnatural, s_0s=[s_0]) #, -1.0, -2.0, -3.0, -9.0, -18.0

        df.loc[len(df)] = result

        k = highDk
        result = run_natural_group(init_params_highDnatural, s_0s=[s_0]) #, -0.2, -1.0, -2.0, -3.0, -9.0, -18.0
